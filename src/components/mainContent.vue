  ...
  <div class="main_content_container">
    ...
    <div v-if="selectedSources.length > 0" class="view_modes_container">
      ...
      >
        ...
        >
          <img
            class="mode_select_button"  :src="'../assets/' + viewMode.image + selectedViewMode === viewMode.id ? '_selected': '' + '.svg'"
          />
        </v-btn>
      </div>
    </div>
<div class="action_buttons" >
    <div           v-for="button in content.actions.footerButtons"      :key="button.id"    >
      <v-btn :class="['footer_button', {selected: button.id=== 'live_button'}]" :id="button.id" depressed>
        {{ button.text }}
      </v-btn>
      </div>
    </div>
